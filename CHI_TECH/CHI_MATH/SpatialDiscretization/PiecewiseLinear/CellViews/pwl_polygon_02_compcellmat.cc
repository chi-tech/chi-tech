...
double PolygonFEView::PreShape(int s, int i, int qpoint_index, bool on_surface)
{
  double xi  = 0.0;
  double eta = 0.0;
  if (!on_surface)
 ...
 * shape function at a quadrature point.*/
double PolygonFEView::PreGradShape_y(int s, int i, int qpoint_index)
{
  int index = node_to_side_map[i][s];
  double value = 0;
  if (index==0)
