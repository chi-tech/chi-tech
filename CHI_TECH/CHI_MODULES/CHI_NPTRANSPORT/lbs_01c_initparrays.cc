...
away from exascale.
   */
int CHI_NPTRANSPORT::InitializeParrays()
{
//  PetscErrorCode ierr;
//  PetscMPIInt    size;
  ...
  for (int c=0; c<grid->local_cell_glob_indices.size(); c++)
  {
    ...
    if (typeid(*cell) == typeid(chi_mesh::CellSlab))
    {
      ...
      NPT_CELLVIEW_FULL* full_cell_view =
        (NPT_CELLVIEW_FULL*)cell_transport_views[slab_cell->cell_local_id];
    ...
    if (typeid(*cell) == typeid(chi_mesh::CellPolygon))
    {
      ...
      NPT_CELLVIEW_FULL* full_cell_view =
        (NPT_CELLVIEW_FULL*)cell_transport_views[poly_cell->cell_local_id];
    ...
    if (typeid(*cell) == typeid(chi_mesh::CellPolyhedron))
    {
      ...
      NPT_CELLVIEW_FULL* full_cell_view =
        (NPT_CELLVIEW_FULL*)cell_transport_views[polyh_cell->cell_local_id];
