...
#include "ChiMath/Quadratures/Conical/quadrature_conical.h"
    ...
    Quadrature(order)
{
  ...
  switch (order)
  {
    ...
    case QuadratureOrder::FOURTH:
    {
      ...
      const double wts[n_wts] =
        {
          double(1.1169079483900573284750350421656140e-01L),
          double(5.4975871827660933819163162450105264e-02L)
        };

      const double a[n_wts] =
        {
          double(4.4594849091596488631832925388305199e-01L),
...
void chi_math::QuadratureTriangle::dunavant_rule(const double rule_data[][4],
...
void chi_math::QuadratureTriangle::dunavant_rule2(const double * wts,
                            const double * a,
                            const double * b,
                            ...
                            unsigned int n_wts)
{
  ...
  for (unsigned int p=0; p<n_wts; ++p)
  {
    switch (permutation_ids[p])
    {
      ...
      default:
        throw std::invalid_argument(std::string(__FUNCTION__) +
        "Unknown permutation id: " + std::to_string(permutation_ids[p]) + "!");
    }
  }
