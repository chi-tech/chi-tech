...
chi_mesh::EdgeLoopCollection* chi_mesh::SurfaceMesh::GetEdgeLoopsPoly()
{
  ...
  //============================================= Loop over faces
  std::vector<chi_mesh::PolyFace*>::iterator cur_face;
  for (cur_face = poly_faces.begin();
       cur_face != poly_faces.end();
       cur_face++)
  {
    //====================================== Loop over edges
    for (int e=0;e<(*cur_face)->edges.size();e++)
    {
      if ((*cur_face)->edges[e][2]<0)
      {
        chi_mesh::Edge new_edge;
        new_edge.v_index[0] = (*cur_face)->edges[e][0];
        new_edge.v_index[1] = (*cur_face)->edges[e][1];

        new_edge.f_index[0] = cur_face - poly_faces.begin();
        new_edge.f_index[2] = std::distance(poly_faces.begin(),cur_face);
