...
IJKArrayDbl WDD_IJK_Sweep(const std::array<size_t, 3>& mesh_divs,
                          ...
                          bool verbose=false)
{
  ...
  for (const auto& omega_n : quad.omegas)
  {
    ...
    else                 iorder = chi_math::Range<int>(Nx-1,-1,-1);

    if (omega_n.y > 0.0) jorder = chi_math::Range<int>(0, Ny);
    else                 jorder = chi_math::Range<int>(Ny-1,-1,-1);

    if (omega_n.z > 0.0) korder = chi_math::Range<int>(0, Nz);
    else                 korder = chi_math::Range<int>(Nz-1,-1,-1);
...
//  q.Set(0.0);
  ...
  auto phi = WDD_IJK_Sweep(mesh_divisions,
                           mesh_lengths,
                           bcs,sigma_t,q,*pquad,
                           verbose);
