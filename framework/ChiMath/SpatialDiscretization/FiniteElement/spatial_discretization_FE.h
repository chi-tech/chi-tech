...
protected:
  explicit SpatialDiscretization_FE(
...
public:
  finite_element::SetupFlags GetSetupFlags() const;
  QuadratureOrder GetQuadratureOrder() const;

public:
  virtual const finite_element::UnitIntegralData&
  ...
  virtual const finite_element::InternalQuadraturePointData&
  GetQPData_Volumetric(const chi_mesh::Cell& cell)
  {
    if (not qp_data_initialized_)
      throw std::invalid_argument(
        "SpatialDiscretization_FE::GetQPData_Volumetric "
        "called without integrals being initialized."
        " Set flag INIT_QP_DATA.");
    return fe_vol_qp_data_[cell.local_id_];
  }

  virtual const finite_element::FaceQuadraturePointData&
  GetQPData_Surface(const chi_mesh::Cell& cell, const unsigned int face)
  {
    if (not qp_data_initialized_)
