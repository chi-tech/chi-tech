...
{
  if (ref_grid_->IsCellLocal(cell.global_id))
  {
    if (integral_data_initialized_)
      return fe_unit_integrals_.at(cell.local_id);
    else
    {
  ...
  else
  {
    if (nb_integral_data_initialized_)
      return nb_fe_unit_integrals_.at(cell.global_id);
    else
    {
      const auto& cell_mapping = GetCellMapping(cell);
      cell_mapping.ComputeUnitIntegrals(scratch_intgl_data_);
  ...
  GetQPData_Volumetric(const chi_mesh::Cell& cell)
{
  if (ref_grid_->IsCellLocal(cell.global_id))
  {
    if (qp_data_initialized_)
      return fe_vol_qp_data_.at(cell.local_id);
    else
    {
      const auto& cell_mapping = GetCellMapping(cell);
  ...
  else
  {
    if (nb_qp_data_initialized_)
      return nb_fe_vol_qp_data_.at(cell.global_id);
    else
    {
      const auto& cell_mapping = GetCellMapping(cell);
      cell_mapping.InitializeVolumeQuadraturePointData(scratch_vol_qp_data_);
                    ...
                    const unsigned int face)
{
  if (ref_grid_->IsCellLocal(cell.global_id))
  {
    if (qp_data_initialized_)
    {
      const auto& face_data = fe_srf_qp_data_.at(cell.local_id);
  ...
  else
  {
    if (nb_qp_data_initialized_)
    {
      const auto& face_data = nb_fe_srf_qp_data_.at(cell.global_id);
